...
async function generateAndCacheInsights(
  ...
  try {
    ...
    const todayAdherence =
      patientContext.patient_health_state?.adherence?.today ?? 0;
    const weeklyAdherenceRaw = patientContext.recent_adherence?.rate
      ? parseInt(patientContext.recent_adherence.rate, 10)
      : 100;
    const weeklyAdherence = isNaN(weeklyAdherenceRaw)
      ? 100
      : weeklyAdherenceRaw;
    const vitalStatus =
    ...
    let computedAdherenceLoss =
      Math.max(0, (100 - todayAdherence) * 0.2) +
    ...
    const sumComponents =
      adherenceLoss + vitalsLoss + moodLoss + visibilityLoss;
    ...
    const roundedSum =
      finalAdherence + finalVitals + finalMood + finalVisibility;
    ...
    const existingInsight = await CompanionAiInsight.findOne({
      patient_id: patientId,
    });
    ...
    if (existingInsight) {
      ...
      trendDelta = {
        ...
        visibility_score:
          visibilityScore - (existingInsight.visibility_score ?? 0),
        confidence_score:
          confidenceScore - (existingInsight.confidence_score ?? 0),
      };
    }
    ...
    if (
      ...
      !skipLlmCall &&
      existingInsight &&
      existingInsight.generated_at
    ) {
      const cacheAgeMs =
        Date.now() - new Date(existingInsight.generated_at).getTime();
      const sameRisk = existingInsight.risk_level === riskLevel;
      const sameActions =
        JSON.stringify(existingInsight.priority_actions || []) ===
        JSON.stringify(priorityActions);
...
async function getOrGenerateInsights(patientId, forceRefresh = false) {
  ...
  if (!forceRefresh) {
    ...
    if (
      cached &&
      cached.generated_at &&
      Date.now() - new Date(cached.generated_at).getTime() < 30 * 60 * 1000
    ) {
      return cached;
    }
  }
