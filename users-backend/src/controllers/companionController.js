 ...
 */
async function joinOtp(req, res) {
  try {
 ...
 */
async function getPatientHealthHistory(req, res) {
  try {
    if (!req.profile || req.profile.role !== 'companion') {
      return res.status(403).json({ error: 'Access denied.' });
    }
 ...
 */
async function getLinkedPatients(req, res) {
  try {
    if (!req.profile || req.profile.role !== 'companion') {
      return res.status(403).json({ error: 'Access denied.' });
    }
 ...
 */
async function requestBp(req, res) {
  try {
    if (!req.profile || req.profile.role !== 'companion') {
      return res.status(403).json({ error: 'Access denied.' });
    }
 ...
 */
async function generatePatientInviteCode(req, res) {
  try {
 ...
 */
async function getAnalyticsExtended(req, res) {
  try {
    if (!req.profile || req.profile.role !== 'companion') {
      return res.status(403).json({ error: 'Access denied.' });
    }
