 ...
 */
async function getTodayMedicines(req, res) {
  try {
 ...
 */
async function markMedicine(req, res) {
  try {
    const {
      medicine_name,
      scheduled_time,
    ...
    const { todayStr, date: today } = getTodayUtcMidnight(patient.timezone);
    const logDate = targetDate
      ? getUtcMidnightFromDateString(targetDate)
      : today;
      ...
      if (patientMed) {
        ...
        if (extMed) {
          ...
          if (
            ...
            if (
              extMed.refillInfo.remainingDoses ===
              (extMed.refillInfo.alertThreshold || 5)
            ) {
              try {
                await Notification.create({
                  patient_id: patient._id,
 ...
 */
async function markSlot(req, res) {
  try {
...
const handleRefillRequest = async (req, res) => {
  try {
    ...
    const addQty = rawQty !== undefined ? Number(rawQty) : 30;
    ...
    if (patientMed) {
      ...
      performRefill(patientMed.refillInfo, addQty);
      await patient.save();
      refilled = true;
    }
    ...
    if (extMed) {
      ...
      performRefill(extMed.refillInfo, addQty);
      await extMed.save();
      refilled = true;
    }
 ...
 */
async function getWeeklyAdherence(req, res) {
  try {
    const patient = await getOrCreatePatient(req);
 ...
 */
async function getMonthlyAdherence(req, res) {
  try {
    const patient = await getOrCreatePatient(req);
...
async function getAdherenceDetails(req, res) {
  try {
    const patient = await getOrCreatePatient(req);
    ...
    let morningRun = 0,
      maxConsecutiveMorning = 0;
    let nightRun = 0,
      maxConsecutiveNight = 0;
    ...
    for (const d of dailyLog) {
      if (d.vitals && (d.vitals.systolic || d.vitals.blood_pressure_systolic)) {
        bpRun++;
        maxConsecutiveBP = Math.max(maxConsecutiveBP, bpRun);
      } else {
        bpRun = 0;
      }
    }
    ...
    const badgeDefs = [
      ...
      },
      {
        ...
        progress:
          maxConsecutiveMorning >= 3
            ? 1
            : Math.min(maxConsecutiveMorning / 3, 0.99),
      ...
      },
      {
        ...
        progress:
          maxConsecutiveNight >= 5
            ? 1
            : Math.min(maxConsecutiveNight / 5, 0.99),
      ...
      },
      {
        ...
        progress:
          scoreImprovement >= 20
            ? 1
            : Math.min(Math.max(scoreImprovement, 0) / 20, 0.99),
      ...
      },
      {
        ...
        progress:
          maxConsecutivePositiveMood >= 3
            ? 1
            : Math.min(maxConsecutivePositiveMood / 3, 0.99),
 ...
 */
async function getAdherenceRecap(req, res) {
  try {
    const patient = await getOrCreatePatient(req);
    ...
    dailyEntries.forEach((entry) => {
      entry.medicines.forEach((m) => {
        ...
        if (!medStats[m.medicine_name])
          medStats[m.medicine_name] = { total: 0, taken: 0 };
    ...
    Object.keys(medStats).forEach((name) => {
      const r =
        medStats[name].total > 0
          ? Math.round((medStats[name].taken / medStats[name].total) * 100)
          : 0;
      if (r > topRate) {
        topRate = r;
        topMed = { name, rate: r };
    ...
    let prevTotal = 0,
      prevTaken = 0;
    prevEntries.forEach((d) => {
      prevTotal += d.total;
      prevTaken += d.taken;
    });
    const prevRate =
      prevTotal > 0 ? Math.round((prevTaken / prevTotal) * 100) : 0;
 ...
 */
async function getTempMeds(req, res) {
  try {
    const patient = await getOrCreatePatient(req);
    const searchIds = [patient._id];
    if (patient.profile_id) searchIds.push(patient.profile_id);
 ...
 */
async function addTempMed(req, res) {
  try {
    const patient = await getOrCreatePatient(req);
 ...
 */
async function deleteTempMed(req, res) {
  try {
    const patient = await getOrCreatePatient(req);
...
module.exports = {
  ...
  getUtcMidnightFromDateString,
  mapTimeToLegacyBucket,
};
