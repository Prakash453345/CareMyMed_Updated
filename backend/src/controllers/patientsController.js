...
const { assignPatientToCaretaker: assignPatientToCaretakerService, unassignPatientFromCaretaker: unassignPatientFromCaretakerService, getPatientCaretakers: getPatientCaretakersService } = require('../services/caretakerService');
const { authorizeMentor: authorizeMentorService, revokeMentorAuthorization: revokeMentorAuthorizationService, getPatientAuthorizedMentors } = require('../services/mentorService');
...
exports.assignPatientToCaretaker = async (req, res) => {
  try {
    ...
    const assignment = await assignPatientToCaretakerService(
      req.profile,
      caretakerId,
      patientId,
      assignmentData
...
exports.unassignPatientFromCaretaker = async (req, res) => {
  try {
    ...
    const assignment = await unassignPatientFromCaretakerService(
      req.profile,
      caretakerId,
      patientId,
      reason,
 ...
 */
exports.getPatientCaretakers = async (req, res) => {
  try {
    const patientId = req.params.id;
    ...
    const caretakers = await getPatientCaretakersService(req.profile, patientId, {
 ...
 */
exports.authorizeMentorForPatient = async (req, res) => {
  try {
    const patientId = req.params.id;
    ...
    const authorization = await authorizeMentorService(
      req.profile,
      mentorId,
      patientId,
      {
 ...
 */
exports.revokeMentorAuthorizationForPatient = async (req, res) => {
  try {
    const { id: patientId, mentorId } = req.params;
    const { reason = '' } = req.body;

    const authorization = await revokeMentorAuthorizationService(
      req.profile,
      mentorId,
      patientId,
      reason,
 ...
 */
exports.getPatientMentors = async (req, res) => {
  try {
    const patientId = req.params.id;
    const { includeInactive = false } = req.query;
...
const syncMedicineLogHelper = async (pId, d, t, mName, isTaken, role) => {
  try {
    ...
    const ptDoc = await Patient.findOne({ $or: [{ _id: pId }, { profile_id: pId }] }).lean();
 ...
 */
exports.toggleMedicationAdherence = async (req, res) => {
  try {
    const patientId = req.params.id;
    ...
    if (!med) {
      ...
      if (!foundMed) {
        fallbackTarget = await Patient.findById(patientId);
      ...
      try {
        let pTarget = await Patient.findById(patientId);
        if (pTarget) {
          let synced = false;
