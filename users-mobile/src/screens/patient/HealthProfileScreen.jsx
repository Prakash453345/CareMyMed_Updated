...
export default function HealthProfileScreen({ navigation }) {
  ...
  const handleWearableSync = async () => {
    if (!healthSdkReady) {
      AlertManager.alert(
        t("common.unsupported", { defaultValue: "Unsupported" }),
        t("health.integration_unavailable", {
          defaultValue: "Health integration is not available on this device.",
        }),
      );
      return;
    }
    ...
    try {
      ...
      if (!hasPermissions) {
        AlertManager.alert(
          ...
          t("health.enable_permissions", {
            defaultValue:
              "Please enable health permissions in your system settings to seamlessly sync vitals.",
          }),
        );
        setIsSyncing(false);
        return;
      ...
      if (vitals.heart_rate || vitals.oxygen_saturation || vitals.systolic) {
        ...
        AlertManager.alert(
          t("common.sync_complete", { defaultValue: "Sync Complete" }),
          t("health.sync_success_desc", {
            defaultValue:
              "Successfully securely pulled your latest smartwatch data into CareMyMed.",
          }),
        );
      } else {
        AlertManager.alert(
          t("common.no_data_found", { defaultValue: "No Data Found" }),
          t("health.no_recent_vitals", {
            defaultValue:
              "We couldn't find any recent vitals recorded by your watch today.",
          }),
        );
      }
    } catch (e) {
      console.error(e);
      AlertManager.alert(
        t("common.sync_error", { defaultValue: "Sync Error" }),
        t("health.sync_error_desc", {
          defaultValue:
            "An error occurred while connecting to your health data.",
        }),
      );
    } finally {
      setIsSyncing(false);
  ...
  const staggerAnims = useRef(
    [...Array(15)].map(() => new Animated.Value(0)),
  ).current;
  ...
  useFocusEffect(
    useCallback(() => {
      const initialProfile =
        usePatientStore.getState().patient || profileRef.current;
  ...
  const openModal = (type, item = null) => {
    ...
    if (item) {
      if (type === "gp" && item.gp_phone) {
        ...
        setFormState({
          ...item,
          gp_phone: parsed.number,
          gp_phoneCode: parsed.code,
        });
      } else {
        setFormState({ ...item });
      }
      ...
      if (type === "condition")
        ...
        setFormState({ name: "", severity: "moderate", reaction: "" });
      else if (type === "vitals")
        setFormState({
          height_cm: profile?.lifestyle?.height_cm || "",
          weight_kg: profile?.lifestyle?.weight_kg || "",
        });
      else if (type === "habits")
        setFormState({
        ...
        setFormState({
          gp_name: profile?.gp?.name || "",
          gp_phone: parsed.number,
          gp_phoneCode: parsed.code,
          gp_email: profile?.gp?.email || "",
        });
      } else if (type === "history")
        setFormState({ event: "", date: "", notes: "" });
        ...
        setFormState({ name: "", administered_by: "" });
  ...
  const executeDelete = async (collection, id) => {
    try {
      ...
      if (Platform.OS === "web")
        window.alert(
          t("health.could_not_delete", {
            defaultValue: "Could not delete item",
          }),
        );
      else
        AlertManager.alert(
      ...
      AlertManager.alert(
        ...
        [
          ...
          },
          {
            ...
            style: "destructive",
            onPress: () => executeDelete(collection, id),
          },
        ],
      );
    }
  ...
  const handleSave = async () => {
    ...
    if (
      editingType === "contact" &&
      formState.name &&
      !nameRegex.test(formState.name)
    ) {
      return AlertManager.alert(
        t("common.invalid_name", { defaultValue: "Invalid Name" }),
        t("health.invalid_contact_name", {
          defaultValue:
            "Contact names can only contain letters, spaces, hyphens, and apostrophes.",
        }),
      );
    }
    if (
      editingType === "gp" &&
      formState.gp_name &&
      !nameRegex.test(formState.gp_name)
    ) {
      return AlertManager.alert(
        t("common.invalid_name", { defaultValue: "Invalid Name" }),
        t("health.invalid_doc_name", {
          defaultValue:
            "Doctor names can only contain letters, spaces, hyphens, and apostrophes.",
        }),
      );
    }
    if (
      ...
      !nameRegex.test(formState.doctor_name)
    ) {
      return AlertManager.alert(
        t("common.invalid_name", { defaultValue: "Invalid Name" }),
        t("health.invalid_doc_name", {
    ...
    if (
      ["condition", "allergy", "medication", "vaccination", "contact"].includes(
        editingType,
      ) &&
      !formState.name?.trim()
    ) {
      return Platform.OS === "web"
        ? window.alert(
            t("health.missing_name", {
              defaultValue: "Please provide a valid name.",
            }),
          )
        : AlertManager.alert(
            t("common.missing_field", { defaultValue: "Missing Field" }),
            t("health.missing_name", {
              defaultValue: "Please provide a valid name.",
            }),
          );
    ...
    if (editingType === "identity" && formState.blood_type) {
      ...
      if (!validTypes.includes(formState.blood_type)) {
        return AlertManager.alert(
          t("health.invalid_blood_type", {
            defaultValue: "Invalid Blood Type",
          }),
          t("health.select_valid_blood", {
            defaultValue: "Please select a valid blood type.",
          }),
        );
      }
    }
    ...
    if (editingType === "history" && !formState.event?.trim()) {
      return Platform.OS === "web"
        ? window.alert(
            t("health.missing_event", {
              defaultValue: "Please provide an event name.",
            }),
          )
        : AlertManager.alert(
            t("common.missing_field", { defaultValue: "Missing Field" }),
    ...
    if (
      editingType === "appointment" &&
      (!formState.title?.trim() || !formState.doctor_name?.trim())
    ) {
      return Platform.OS === "web"
        ? window.alert(
            t("health.missing_appt_details", {
              defaultValue: "Please provide appointment details.",
            }),
          )
        : AlertManager.alert(
            t("common.missing_field", { defaultValue: "Missing Field" }),
    ...
    if (
      ["condition", "allergy", "medication", "history"].includes(editingType)
    ) {
      const val = formState.name || formState.event;
      if (val && val.trim().length < 2) {
        return AlertManager.alert(
          t("common.too_short", { defaultValue: "Too Short" }),
          t("health.name_too_short", {
            defaultValue:
              "Please enter a more descriptive name (at least 2 characters).",
          }),
        );
      }
    }
    ...
    if (
      ["history", "condition", "allergy"].includes(editingType) &&
      formState.date
    ) {
      if (new Date(formState.date) > new Date()) {
        return AlertManager.alert(
          t("common.invalid_date", { defaultValue: "Invalid Date" }),
          t("health.date_future_error", {
            defaultValue: "Date cannot be in the future.",
          }),
        );
      }
    }
    if (editingType === "vaccination" && formState.date_given) {
      if (new Date(formState.date_given) > new Date()) {
        return AlertManager.alert(
          t("common.invalid_date", { defaultValue: "Invalid Date" }),
          t("health.vaccine_future_error", {
            defaultValue: "Vaccination date cannot be in the future.",
          }),
        );
      }
    }
    if (editingType === "appointment" && formState.date) {
      ...
      if (new Date(formState.date) < today && formState.status === "upcoming") {
        return AlertManager.alert(
          t("common.invalid_date", { defaultValue: "Invalid Date" }),
          t("health.appt_past_error", {
            defaultValue: "Upcoming appointment date cannot be in the past.",
          }),
        );
      }
    }
    ...
    if (editingType === "gp" && formState.gp_phone) {
      const phoneErr = validatePhone(
        formState.gp_phone,
        formState.gp_phoneCode,
      );
      if (phoneErr)
        return AlertManager.alert(
          t("common.invalid_phone", { defaultValue: "Invalid Phone" }),
          phoneErr,
        );
    }
    ...
    if (editingType === "vitals") {
        ...
        return Platform.OS === "web"
          ? window.alert(
              t("health.height_range", {
                defaultValue: "Height must be between 50–300 cm.",
              }),
            )
          : AlertManager.alert(
              t("common.invalid_height", { defaultValue: "Invalid Height" }),
              t("health.height_range", {
                defaultValue: "Height must be between 50 and 300 cm.",
              }),
            );
      }
        ...
        return Platform.OS === "web"
          ? window.alert(
              t("health.weight_range", {
                defaultValue: "Weight must be between 10–500 kg.",
              }),
            )
          : AlertManager.alert(
              t("common.invalid_weight", { defaultValue: "Invalid Weight" }),
              t("health.weight_range", {
                defaultValue: "Weight must be between 10 and 500 kg.",
              }),
            );
      }
      if (!isNaN(h) && !isNaN(w) && h > 0 && w > 0) {
        ...
        if (bmi < 10 || bmi > 60) {
          return AlertManager.alert(
            t("health.invalid_vitals", { defaultValue: "Invalid Vitals" }),
            t("health.invalid_bmi_desc", {
              defaultValue: `The calculated BMI of ${bmi.toFixed(1)} seems highly unlikely. Please verify your height and weight inputs.`,
              bmi: bmi.toFixed(1),
            }),
          );
        }
    ...
    if (!formState._id) {
      ...
      if (editingType === "condition" && checkDuplicate(conditions, "name")) {
        return Platform.OS === "web"
          ? window.alert(
              t("health.condition_exists", {
                defaultValue: "This condition already exists.",
              }),
            )
          : AlertManager.alert(
              t("common.duplicate", { defaultValue: "Duplicate" }),
              t("health.condition_exists_desc", {
                defaultValue:
                  "This condition already exists in your health profile.",
              }),
            );
      }
      if (editingType === "allergy" && checkDuplicate(allergies, "name")) {
        return Platform.OS === "web"
          ? window.alert(
              t("health.allergy_exists", {
                defaultValue: "This allergy already exists.",
              }),
            )
          : AlertManager.alert(
              t("common.duplicate", { defaultValue: "Duplicate" }),
              t("health.allergy_exists_desc", {
                defaultValue:
                  "This allergy already exists in your health profile.",
              }),
            );
      }
      if (editingType === "medication" && checkDuplicate(medications, "name")) {
        return Platform.OS === "web"
          ? window.alert(
              t("health.med_exists", {
                defaultValue: "This medication already exists.",
              }),
            )
          : AlertManager.alert(
              t("common.duplicate", { defaultValue: "Duplicate" }),
              t("health.med_exists_desc", {
                defaultValue:
                  "This medication already exists in your health profile.",
              }),
            );
      }
      if (
        editingType === "vaccination" &&
        checkDuplicate(vaccinations, "name")
      ) {
        return Platform.OS === "web"
          ? window.alert(
              t("health.vax_exists", {
                defaultValue: "This vaccination already exists.",
              }),
            )
          : AlertManager.alert(
              t("common.duplicate", { defaultValue: "Duplicate" }),
              t("health.vax_exists_desc", {
                defaultValue:
                  "This vaccination already exists in your health profile.",
              }),
            );
      }
      if (
        ...
        return Platform.OS === "web"
          ? window.alert(
              t("health.history_exists", {
                defaultValue: "This medical history entry already exists.",
              }),
            )
          : AlertManager.alert(
              t("common.duplicate", { defaultValue: "Duplicate" }),
              t("health.history_exists_desc", {
                defaultValue:
                  "This entry already exists in your medical history.",
              }),
            );
      }
    }
    ...
    try {
      ...
      if (["vitals", "habits", "activity", "identity"].includes(editingType)) {
        let aids = [];
        let diets = [];
        if (formState.mobility_aids)
          aids = formState.mobility_aids
            .split(",")
            .map((s) => s.trim())
            .filter((s) => s);
        if (formState.dietary_restrictions)
          diets = formState.dietary_restrictions
            .split(",")
            .map((s) => s.trim())
            .filter((s) => s);
        payload = {
      ...
      if (editingType === "condition")
        res = await apiService.patients.updateConditions(payload);
      else if (editingType === "allergy")
        res = await apiService.patients.updateAllergies(payload);
      else if (
        ["vitals", "habits", "activity", "identity"].includes(editingType)
      ) {
        ...
        if (formState._id)
          res = await apiService.patients.updateTrustedContact(
            formState._id,
            payload,
          );
        else res = await apiService.patients.addTrustedContact(payload);
      } else if (editingType === "gp")
        res = await apiService.patients.updatePrimaryDoctor(payload);
      else if (editingType === "history")
        res = await apiService.patients.updateMedicalHistory({
          ...payload,
          date: payload.date ? new Date(payload.date) : new Date(),
        });
      else if (editingType === "medication")
        res = await apiService.patients.updateMedications(payload);
      else if (editingType === "vaccination")
        res = await apiService.patients.updateVaccinations({
          ...payload,
          date_given: payload.date_given
            ? new Date(payload.date_given)
            : new Date(),
        });
      else if (editingType === "appointment")
        res = await apiService.patients.updateAppointments({
          ...payload,
          date: payload.date ? new Date(payload.date) : new Date(),
        });
      ...
      if (Platform.OS === "web")
        window.alert("Failed to save data. Please check your inputs.");
      else
        AlertManager.alert(
          "Error",
          "Failed to save data. Please check your inputs.",
        );
      console.warn(error);
    } finally {
      setIsSaving(false);
  ...
  if (loading) {
    return (
      <View
        style={[
          s.container,
          { padding: 20, paddingTop: Platform.OS === "android" ? 60 : 40 },
        ]}
      >
        <SkeletonItem
          width={160}
      ...
      <View style={[s.container, { justifyContent: "center", padding: 24 }]}>
        ...
        >
          ...
          <Text style={s.premiumBannerTitle}>
            {t("health_profile.free_title", {
              defaultValue: "Your health story starts here ❤️",
            })}
          </Text>
          <Text style={s.premiumBannerBody}>
            {t("health_profile.free_body", {
              defaultValue:
                "Keep your medical history, care contacts, medications, and important records organized in one secure place.",
            })}
          </Text>
          ...
          <Text style={s.premiumBannerPitch}>
            {t("health_profile.free_pitch", {
              defaultValue:
                "Upgrade anytime to unlock advanced health insights, AI trend analysis, and personalized forecasting.",
            })}
          </Text>
          ...
          >
            ...
            <Text style={s.premiumCtaBtnTxt}>
              {t("health_profile.unlock_premium", {
                defaultValue: "Unlock Premium Insights",
              })}
            </Text>
          </Pressable>
        </LinearGradient>
  ...
  const medical_history = Array.isArray(rawMedicalHistory)
    ? rawMedicalHistory
    : [];
  ...
  const getBmiStyle = (val) => {
    ...
    if (v < 18.5)
      return {
        label: t("health.underweight", { defaultValue: "Underweight" }),
      };
    if (v < 25)
      return { label: t("common.normal", { defaultValue: "Normal" }) };
    if (v < 30)
      return { label: t("health.overweight", { defaultValue: "Overweight" }) };
  ...
  const calcCompletion = () => {
    ...
    if (gp.name) score++;
    if (lifestyle.smoking_status && lifestyle.smoking_status !== "never")
      score++;
    else if (lifestyle.smoking_status === "never") score++;
  ...
  const handleCompletionClick = () => {
    ...
    if (!profile?.blood_type || profile.blood_type === "unknown")
      missing.push(
        t("health_profile.blood_type", { defaultValue: "Blood Type" }),
      );
    ...
    if (allergies.length === 0)
      missing.push(
        t("health_profile.allergies", { defaultValue: "Allergies" }),
      );
    if (medical_history.length === 0)
      missing.push(
        t("health_profile.medical_history", {
          defaultValue: "Medical History",
        }),
      );
    if (medications.length === 0)
      missing.push(
        t("health_profile.medications", { defaultValue: "Medications" }),
      );
    if (vaccinations.length === 0)
      missing.push(
        t("health_profile.vaccinations", { defaultValue: "Vaccinations" }),
      );
    if (!lifestyle.height_cm || !lifestyle.weight_kg)
      missing.push(
        t("health_profile.height_weight", { defaultValue: "Height & Weight" }),
      );
    if (!profile?.trusted_contacts?.length)
      missing.push(
        t("caller.emergency_contact", { defaultValue: "Emergency Contact" }),
      );
    if (!gp.name)
      missing.push(
        t("health_profile.primary_gp", { defaultValue: "Primary GP" }),
      );
    if (!lifestyle.smoking_status || lifestyle.smoking_status === "unknown")
      missing.push(
        t("health_profile.smoking_status", { defaultValue: "Smoking Status" }),
      );
    if (missing.length === 0) {
      AlertManager.alert(
        t("health_profile.profile_complete", {
          defaultValue: "Profile Complete",
        }),
        t("health_profile.profile_complete_desc", {
          defaultValue:
            "You have completed 100% of your health profile. Great job!",
        }),
      );
    } else {
      AlertManager.alert(
        t("health_profile.incomplete_profile", {
          defaultValue: "Incomplete Profile",
        }),
        t("health_profile.incomplete_profile_desc", {
          defaultValue:
  ...
  const hsScore =
    _rawScore !== undefined && _rawScore !== null && !isNaN(Number(_rawScore))
      ? Number(_rawScore)
      : null;
  const hsLabel =
    hs?.label ?? t("health_profile.status_stable", { defaultValue: "Stable" });
  const hsColor =
    hs?.color === "#3B82F6" || hs?.color === "#0EA5E9"
      ? "#7C3AED"
      : (hs?.color ?? "#7C3AED");
  ...
  const bracketLabel =
    {
      young_adult: t("health_profile.bracket_young", {
        defaultValue: "Young Adult",
      }),
      middle_aged: t("health_profile.bracket_middle", {
        defaultValue: "Middle Aged",
      }),
  ...
  const formatLastComputed = (iso) => {
    if (!iso) {
      return hsScore !== null || hsGrade !== "—"
        ? t("health_profile.updated_recently", {
            defaultValue: "Updated recently",
          })
        : t("health_profile.last_sync_unknown", {
            defaultValue: "Not yet computed",
          });
    }
    const d = new Date(iso);
    const diffMin = Math.floor((Date.now() - d.getTime()) / 60000);
    if (diffMin < 1)
      return t("health_profile.just_now", { defaultValue: "Updated just now" });
    if (diffMin < 60)
      return t("health_profile.mins_ago", {
        defaultValue: "{{n}}m ago",
        n: diffMin,
      }).replace("{{n}}", diffMin);
    const diffH = Math.floor(diffMin / 60);
    if (diffH < 24)
      return t("health_profile.hours_ago", {
  ...
  const calcHabitScore = () => {
    let score = 50;
    if (lifestyle.smoking_status === "never") score += 20;
    else if (lifestyle.smoking_status === "former") score += 10;
    else if (lifestyle.smoking_status === "current") score -= 10;
    if (lifestyle.alcohol_use === "none") score += 15;
    else if (lifestyle.alcohol_use === "occasional") score += 5;
    else score -= 10;
    if (lifestyle.exercise_frequency === "active") score += 15;
    else if (lifestyle.exercise_frequency === "moderate") score += 10;
    else if (lifestyle.exercise_frequency === "light") score += 5;
    return Math.max(0, Math.min(100, score));
  };
  const habitScore = calcHabitScore();
  const habitLabel =
    habitScore >= 70
      ? t("common.good", { defaultValue: "Good" })
      : habitScore >= 40
        ? t("common.fair", { defaultValue: "Fair" })
        : t("health.needs_work", { defaultValue: "Needs Work" });
  const habitColor =
    habitScore >= 70 ? "#10B981" : habitScore >= 40 ? "#F59E0B" : "#EF4444";

  const managedCount = conditions.filter(
    (c) => c.status === "managed" || c.status === "resolved",
  ).length;
  const trendLabel =
    conditions.length === 0
      ? t("common.good", { defaultValue: "Good" })
      : managedCount >= conditions.length * 0.5
        ? t("common.good", { defaultValue: "Good" })
        : t("health.monitor", { defaultValue: "Monitor" });
  const trendSub =
    conditions.length === 0
      ? t("health.no_issues", { defaultValue: "No issues" })
      : managedCount >= conditions.length * 0.5
        ? t("health.stable", { defaultValue: "Stable" })
        : t("health.attention", { defaultValue: "Attention" });
  const trendColor =
    trendLabel === t("common.good", { defaultValue: "Good" })
      ? "#10B981"
      : "#F59E0B";
  ...
  const statusOptions = [
    ...
    {
      label: t("health.managed", { defaultValue: "Managed" }),
      value: "managed",
    },
    {
      label: t("health.resolved_cured", { defaultValue: "Resolved/Cured" }),
      value: "resolved",
    },
  ];
  const smokeOptions = [
    {
      label: t("health.non_smoker", { defaultValue: "Non-Smoker" }),
      value: "never",
    },
    { label: t("health.smoker", { defaultValue: "Smoker" }), value: "current" },
    { label: t("health.former", { defaultValue: "Former" }), value: "former" },
  ];
  const alcoholOptions = [
    {
      label: t("health.non_drinker", { defaultValue: "Non-Drinker" }),
      value: "none",
    },
    {
      label: t("health.occasional", { defaultValue: "Occasional" }),
      value: "occasional",
    },
    {
      label: t("health.frequent", { defaultValue: "Frequent" }),
      value: "heavy",
    },
  ];
  const exerciseOptions = [
    {
      label: t("health.no_activity", { defaultValue: "No Activity" }),
      value: "none",
    },
    {
      label: t("health.light_activity", {
        defaultValue: "Light (Walks, Stretching)",
      }),
      value: "light",
    },
    {
      label: t("health.moderate_activity", {
        defaultValue: "Moderate (Gym, Jogging)",
      }),
      value: "moderate",
    },
    {
      label: t("health.highly_active", {
        defaultValue: "Highly Active (Heavy Cardio)",
      }),
      value: "active",
    },
  ];
  const mobilityOptions = [
    { label: t("health.full", { defaultValue: "Full" }), value: "full" },
    {
      label: t("health.limited", { defaultValue: "Limited" }),
      value: "limited",
    },
    {
      label: t("health.wheelchair", { defaultValue: "Wheelchair" }),
      value: "wheelchair",
    },
    {
      label: t("health.bedridden", { defaultValue: "Bedridden" }),
      value: "bedridden",
    },
  ];
  const frequencyOptions = [
    { label: t("health.daily", { defaultValue: "Daily" }), value: "daily" },
    { label: t("health.weekly", { defaultValue: "Weekly" }), value: "weekly" },
    {
      label: t("health.as_needed", { defaultValue: "As Needed" }),
      value: "as_needed",
    },
  ];
  const timeOptions = [
    {
      label: t("time_slots.morning", { defaultValue: "Morning" }),
      value: "morning",
    },
    {
      label: t("time_slots.afternoon", { defaultValue: "Afternoon" }),
      value: "afternoon",
    },
    {
      label: t("time_slots.evening", { defaultValue: "Evening" }),
      value: "evening",
    },
    { label: t("time_slots.night", { defaultValue: "Night" }), value: "night" },
  ];
  const bloodOptions = [
    { label: "A+", value: "A+" },
    { label: "A-", value: "A-" },
    ...
    { label: "O-", value: "O-" },
    {
      label: t("health.unknown_blood", { defaultValue: "Unknown" }),
      value: "unknown",
    },
  ];
    ...
    <TabScreenTransition>
      <View style={s.root}>
        ...
        <View ref={headerRef} collapsable={false} style={s.header}>
          <View style={s.headerRow}>
            <View style={{ flex: 1 }}>
              ...
              >
                {t("health_profile.overview_sub", {
                  defaultValue:
                    "Overview of your health and medical information",
                })}
              </Text>
            </View>
            <Pressable
        ...
        >
                ...
                >
                    ...
                    >
                      <Text
                        style={[
                          ...
                          {
                            color:
                              completionPct === 100 ? "#10B981" : "#7C3AED",
                          },
                        ]}
                      >
                        {completionPct}%
                      ...
                      <ChevronDown
                        ...
                        style={{
                          transform: [
                            {
                              rotate: completenessExpanded ? "180deg" : "0deg",
                            },
                          ],
                        }}
                      />
                  ...
                  {!completenessExpanded && completionPct < 100 && (
                    <Text style={s.completeBannerSub}>
                      {t("health_profile.tap_to_complete", {
                        defaultValue: "Tap to see what’s missing →",
                      })}
                    </Text>
                  )}
                </Pressable>
                {completenessExpanded && (
                  <View style={[s.checklistContainer, { marginTop: 12 }]}>
                    {[
                      {
                        key: "blood_type",
                        label: t("health_profile.blood_type", {
                      ...
                      {
                        key: "conditions",
                        label: t("health_profile.current_conditions", {
                          defaultValue: "Current Conditions",
                        }),
                        completed: conditions.length > 0,
                      },
                      {
                        key: "allergies",
                        label: t("health_profile.allergies", {
                      ...
                      {
                        key: "medical_history",
                        label: t("health_profile.medical_history", {
                          defaultValue: "Medical History",
                        }),
                        completed: medical_history.length > 0,
                      },
                      {
                        key: "medications",
                        label: t("health_profile.medications", {
                      ...
                      {
                        key: "vaccinations",
                        label: t("health_profile.vaccinations", {
                      ...
                      {
                        key: "lifestyle",
                        label: t("health_profile.height_weight", {
                          defaultValue: "Height & Weight",
                        }),
                        completed: !!(
                          lifestyle.height_cm && lifestyle.weight_kg
                        ),
                      },
                      {
                        key: "trusted_contacts",
                        label: t("caller.emergency_contact", {
                          defaultValue: "Emergency Contact",
                        }),
                        completed: !!(profile?.trusted_contacts?.length > 0),
                      },
                      {
                        key: "gp",
                        label: t("health_profile.primary_gp", {
                          defaultValue: "Primary GP",
                        }),
                        completed: !!gp.name,
                      },
                      {
                        key: "smoking",
                        label: t("health_profile.smoking_status", {
                          defaultValue: "Smoking Status",
                        }),
                        completed: !!(
                          lifestyle.smoking_status &&
                          lifestyle.smoking_status !== "unknown"
                      ...
                      return (
                        ...
                        >
                          ...
                          <Text
                            style={[
                              s.checklistText,
                              item.completed && s.checklistTextCompleted,
                            ]}
                            numberOfLines={1}
                          >
                            {item.label}
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                if (isLocked) {
                  ...
                  AlertManager.alert(
                    t("health_profile.unlock_score_title", {
                      defaultValue: "Unlock Your Health Insights",
                    }),
                    t("health_profile.unlock_score_desc", {
                      defaultValue:
                        "Complete at least 50% of your health profile to unlock your personalized health score, biological wellness estimate, and AI coaching. You're on your way!",
                    }),
                  );
              ...
              >
                <View style={s.dashTopRow}>
                  <View style={s.dashLeft}>
                    ...
                    >
                      <Text style={s.dashEyebrow}>
                        {t("health_profile.health_score", {
                          defaultValue: "HEALTH SCORE",
                        })}
                      </Text>
                      <Pressable
                          ...
                          {hsGrade !== "—" && (
                            <View
                              style={[
                                s.gradeChip,
                                {
                                  backgroundColor: hsColor + "20",
                                  borderColor: hsColor,
                                },
                              ]}
                            >
                              <Text
                                style={[s.gradeChipTxt, { color: hsColor }]}
                              >
                                {hsGrade}
                              </Text>
                            </View>
                    ...
                    <View style={s.dashStatusRow}>
                      {completionPct >= 50 ? (
                        <>
                          ...
                          <Text style={[s.dashStatusTxt, { color: hsColor }]}>
                            {hsLabel}
                          </Text>
                        </>
                      ) : (
                        <>
                          ...
                          <Text style={[s.dashStatusTxt, { color: "#94A3B8" }]}>
                            Building Profile
                          </Text>
                        </>
                      )}
                    ...
                    {hsBracket && completionPct >= 50 && (
                      <View style={s.bracketTag}>
                        <Text style={s.bracketTagTxt}>
                          {t("health_profile.adjusted_for", {
                            defaultValue: "Adjusted for age • {{bracket}}",
                            bracket: bracketLabel,
                          }).replace("{{bracket}}", bracketLabel)}
                        </Text>
                      </View>
                    )}
                    <Pressable
                    ...
                    >
                      <RefreshCw
                        size={10}
                        color={isSyncing ? colors.primary : "#94A3B8"}
                        style={
                          isSyncing ? { transform: [{ rotate: "45deg" }] } : {}
                        }
                      />
                      <Text
                        style={[
                          s.dashSyncTxt,
                          isSyncing && { color: colors.primary },
                        ]}
                      >
                        {isSyncing
                          ? t("health_profile.syncing", {
                    ...
                    <View style={s.ringWrap}>
                      <Svg width={88} height={88} viewBox="0 0 88 88">
                        <SvgCircle
                          ...
                          cy="44"
                          r="38"
                          stroke={completionPct < 50 ? "#F3E8FF" : "#F3E8FF"}
                          strokeWidth="8"
                          fill="transparent"
                        />
                        <SvgCircle
                      ...
                      {completionPct < 50 ? (
                        ...
                        <HeartPulse
                          size={24}
                          color={hsScore !== null ? hsColor : "#94A3B8"}
                ...
                >
                  <Text
                    style={{ fontSize: 12, color: "#94A3B8", ...FONT.bold }}
                  >
                    {completionPct < 50
                      ? "Tap to view checklist"
                      : "Tap for full breakdown & insights"}
                  </Text>
                  <ChevronRight
                    size={14}
                    color="#94A3B8"
          ...
          <Animated.View style={anim(1)}>
            <View ref={alertsCardRef} collapsable={false} style={s.alertsCard}>
              <Pressable
                ...
                onPress={() => {
                  AlertManager.alert(
                    t("health_profile.manage_alerts", {
                      defaultValue: "Manage Health Alerts",
                    }),
                    t("health_profile.manage_alerts_desc", {
                      defaultValue:
                        "Manage active conditions and severe allergies",
                    }),
                    [
                      {
                        text: t("health_profile.add_condition", {
                          defaultValue: "Add Condition",
                        }),
                        onPress: () => openModal("condition"),
                      },
                      {
                        text: t("health_profile.add_allergy", {
                          defaultValue: "Add Allergy",
                        }),
                  ...
                  <Text style={s.alertSub}>
                    {conditions.filter((c) => c.status === "active").length +
                      allergies.filter((a) => a.severity === "severe")
                        .length}{" "}
                    {t("health_profile.active_alerts_sub", {
                      defaultValue: "active alerts need your attention",
                    })}
                  </Text>
                </View>
              ...
              <View style={s.alertChips}>
                ...
                {allergies
                  ...
                  .map((a, i) => (
                    ...
                    >
                      <View style={s.alertDot} />
                      <Text style={s.alertChipTxt}>
                        {a.name}{" "}
                        {t("health_profile.allergy", {
          ...
          <Animated.View style={anim(1.5)}>
            <View style={s.bentoGrid}>
              <Pressable
                style={s.bentoCard}
                onPress={() => openModal("identity")}
              >
                <View style={[s.bentoCircle, { backgroundColor: "#FEE2E2" }]}>
          ...
          >
            ...
            <Animated.View style={anim(2)}>
              <View ref={medicalRecordsCardRef} collapsable={false} style={s.gridCard}>
                ...
                <View style={s.gridBody}>
                  {conditions.map((c, i) => {
                    const statStyle =
                      CONDITION_STATUS[c.status] || CONDITION_STATUS.active;
                    return (
                      <Pressable
                        key={i}
                        style={s.gridRow}
                      ...
                      >
                        <View
                          style={[
                            s.gridDot,
                            { backgroundColor: statStyle.text },
                          ]}
                        />
                        <Text style={s.gridRowTxt} numberOfLines={1}>
                          {c.name}
                        </Text>
                        <View
                          style={[
                        ...
                        >
                          <Text
                            style={[s.miniPillTxt, { color: statStyle.text }]}
                          >
                            {t(`health_profile.status_${c.status}`, {
                  ...
                  {conditions.length === 0 && (
                    <Text style={s.emptyGridTxt}>
                      {t("health_profile.no_conditions", {
                        defaultValue: "No conditions",
                      })}
                    </Text>
                  )}
                </View>
            ...
            <Animated.View style={anim(3)}>
              <View style={s.gridCard}>
                <View style={s.gridHeader}>
                  <View
                    style={[s.gridIconWrap, { backgroundColor: "#FEF3C7" }]}
                  >
                    <AlertTriangle size={16} color="#F59E0B" />
                  </View>
                  <Text style={s.gridTitle}>
                    {t("health_profile.allergies", {
                      defaultValue: "Allergies",
                    })}
                  </Text>
                  <Pressable
            ...
            <Animated.View style={anim(4)}>
              <View style={s.gridCard}>
                <View style={s.gridHeader}>
                  <View
                    style={[s.gridIconWrap, { backgroundColor: "#D1FAE5" }]}
                  >
                    <Activity size={16} color="#10B981" />
                  </View>
                  <Text style={s.gridTitle}>
                    {t("health_profile.wellness", { defaultValue: "Wellness" })}
                  </Text>
                  <Pressable
                    style={s.gridAddBtn}
                    onPress={() => openModal("vitals")}
                ...
                <View
                  style={[
                    s.gridBody,
                    { flexDirection: "row", gap: 10, paddingBottom: 6 },
                  ]}
                >
                  <Pressable
                    style={[s.wellBox, { borderColor: "#FEF3C7" }]}
                    onPress={() => openModal("vitals")}
                  >
                    <Text
                      style={[s.wellVal, { color: "#F59E0B" }]}
                      adjustsFontSizeToFit
                      numberOfLines={1}
                    >
                      {bmi || "—"}
                    </Text>
                    <Text style={s.wellLbl}>
                      {t("health_profile.bmi", { defaultValue: "BMI" })}
                    </Text>
                    <Text style={[s.wellSub, { color: "#F59E0B" }]}>
                      {bmiTheme.label}
                    </Text>
                  </Pressable>
                  <Pressable
                    style={[s.wellBox, { borderColor: "#D1FAE5" }]}
                    onPress={() => openModal("habits")}
                  >
                    <Text
                      style={[s.wellVal, { color: "#10B981" }]}
                      adjustsFontSizeToFit
                      numberOfLines={1}
                    >
                      {habitScore}%
                    ...
                    <Text style={s.wellLbl}>
                      {t("health_profile.habits", { defaultValue: "Habits" })}
                    </Text>
                    <Text style={[s.wellSub, { color: "#10B981" }]}>
                      {habitLabel}
                    </Text>
                  </Pressable>
                  <Pressable
                  ...
                  >
                    <Text
                      ...
                      adjustsFontSizeToFit
                    >
                      {lifestyle.mobility_level
                        ? lifestyle.mobility_level.charAt(0).toUpperCase() +
                          lifestyle.mobility_level.slice(1)
                        : "—"}
                    </Text>
                    <Text
                      style={s.wellLbl}
                      ...
                      adjustsFontSizeToFit
                    >
                      {t("health_profile.mobility", {
                        defaultValue: "Mobility",
                      })}
                    </Text>
                    <Text
                      style={[s.wellSub, { color: "#10B981" }]}
                      numberOfLines={1}
                      adjustsFontSizeToFit
                    >
                      {lifestyle.exercise_frequency
                        ? t(
                            `health_profile.intensity_${lifestyle.exercise_frequency}`,
                            {
                              defaultValue:
                                lifestyle.exercise_frequency
                                  .charAt(0)
                                  .toUpperCase() +
                                lifestyle.exercise_frequency.slice(1),
                            },
                          )
                        : "—"}
                    </Text>
            ...
            <Animated.View style={anim(5)}>
              <View style={s.gridCard}>
                <View style={s.gridHeader}>
                  <View
                    style={[s.gridIconWrap, { backgroundColor: "#F3E8FF" }]}
                  >
                    <Pill size={16} color="#8B5CF6" />
                  </View>
                  <Text style={s.gridTitle}>
                    {t("health_profile.medications", {
                      defaultValue: "Medications",
                    })}
                  </Text>
                  <Pressable
                ...
                <View style={s.gridBody}>
                  {["morning", "afternoon", "evening", "night"].map((time) => {
                    const meds = activeMeds.filter((m) =>
                      m.times?.includes(time),
                    );
                    if (meds.length === 0) return null;
                    return (
                      <View key={time} style={{ marginBottom: 12 }}>
                        <Text style={s.gridTimeLbl}>
                          {t(`health_profile.time_${time}`, {
                            defaultValue:
                        ...
                        {meds.map((m, idx) => (
                          <Pressable
                            ...
                            onPress={() => openModal("medication", m)}
                          >
                            <View style={s.tinyDot} />
                            <Text style={s.gridMedTxt} numberOfLines={1}>
                              {m.name}{" "}
                              <Text
                  ...
                  {activeMeds.length === 0 && (
                    <Text style={s.emptyGridTxt}>
                      {t("health_profile.no_meds", {
                        defaultValue: "No active meds",
                      })}
                    </Text>
                  )}
                </View>
            ...
            <Animated.View style={anim(6)}>
              <View style={s.gridCard}>
                <View style={s.gridHeader}>
                  <View
                    style={[s.gridIconWrap, { backgroundColor: "#FCE7F3" }]}
                  >
                    <Syringe size={16} color="#EC4899" />
                  </View>
                  <Text style={s.gridTitle}>
                    {t("health_profile.vaccinations", {
                      defaultValue: "Vaccinations",
                    })}
                  </Text>
                  <Pressable
                ...
                <View style={s.gridBody}>
                  {vaccinations.map((v, i) => (
                    <Pressable
                      ...
                      onPress={() => openModal("vaccination", v)}
                    >
                      <View style={{ flex: 1 }}>
                  ...
                  {vaccinations.length === 0 && (
                    <Text style={s.emptyGridTxt}>
                      {t("health_profile.no_vaccinations", {
                        defaultValue: "No vaccinations logged",
                      })}
                    </Text>
                  )}
                </View>
            ...
            <Animated.View style={anim(7)}>
              <View style={s.gridCard}>
                <View style={s.gridHeader}>
                  ...
                  <Text style={s.gridTitle}>
                    {t("health_profile.medical_history_count", {
                      defaultValue: "Medical History ({{count}} records)",
                      count: medical_history.length,
                    }).replace("{{count}}", String(medical_history.length))}
                  </Text>
                  <Pressable
                    style={s.gridAddBtn}
                  ...
                  {medical_history.length > 3 && (
                    ...
                    >
                      ...
                      {visibleHistoryCount < medical_history.length && (
                        ...
                        >
                          <Text style={s.historyLinkBtnTxt}>
                            {t("health_profile.show_more_count", {
                              defaultValue: "Show {{count}} More",
                              count: Math.min(
                                4,
                                medical_history.length - visibleHistoryCount,
                              ),
                            }).replace(
                              "{{count}}",
                              String(
            ...
            <Animated.View style={anim(8)}>
              <View style={s.gridCard}>
                <View style={s.gridHeader}>
                  ...
                  <Text style={s.gridTitle}>
                    {t("health_profile.care_network", {
                      defaultValue: "Care Network",
                    })}
                  </Text>
                  <Pressable
                    style={s.gridAddBtn}
                    onPress={() => {
                      AlertManager.alert(
                        t("health_profile.add_to_network", {
                          defaultValue: "Add to Care Network",
                        }),
                        t("health_profile.add_network_desc", {
                          defaultValue: "Choose who you would like to add",
                        }),
                        [
                          {
                            text: t("health_profile.primary_doctor", {
                              defaultValue: "Primary Doctor",
                            }),
                            onPress: () => openModal("gp"),
                          },
                          {
                            text: t("caller.emergency_contact", {
                              defaultValue: "Emergency Contact",
                            }),
                  ...
                  {gp.name && (
                    <View style={s.netRow}>
                      ...
                      >
                        ...
                        <Text style={s.netRole}>
                          {t("health_profile.primary_doctor", {
                            defaultValue: "Primary Doctor",
                          })}
                        </Text>
                      </Pressable>
                      <Pressable
                        ...
                        onPress={() =>
                          gp.phone && Linking.openURL(`tel:${gp.phone}`)
                        }
                      >
                        <Phone
                          size={12}
                          ...
                          style={{ marginRight: 4 }}
                        />
                        <Text style={[s.netBtnTxt, { color: "#8B5CF6" }]}>
                          {t("health_profile.call", { defaultValue: "Call" })}
                        </Text>
                      </Pressable>
                    </View>
                  )}
                  {profile?.trusted_contacts
                    ...
                    .map((c, i) => (
                      <View key={"c" + i} style={[s.netRow, { marginTop: 10 }]}>
                        <Pressable
                          style={{ flex: 1, paddingRight: 6 }}
                          onPress={() => openModal("contact", c)}
                        >
                          <Text style={s.netName} numberOfLines={1}>
                            {c.name}
                          </Text>
                          <Text style={s.netRole}>
                            {t("health_profile.emergency_contact", {
                              defaultValue: "Emergency Contact",
                            })}
                          </Text>
                        </Pressable>
                        ...
                        >
                          <Siren
                            ...
                            style={{ marginRight: 4 }}
                          />
                          <Text style={[s.netBtnTxt, { color: "#EF4444" }]}>
                            {t("health_profile.sos", { defaultValue: "SOS" })}
                          </Text>
                        </Pressable>
                      </View>
                    ))}
                  {!gp.name && !profile?.trusted_contacts?.length && (
                    <Text style={s.emptyGridTxt}>
                      {t("health_profile.no_contacts", {
            ...
            <Animated.View style={anim(9)}>
              <View style={s.gridCard}>
                <View style={s.gridHeader}>
                  <View
                    style={[s.gridIconWrap, { backgroundColor: "#FAF5FF" }]}
                  >
                    <Calendar size={16} color="#8B5CF6" />
                  </View>
                  <Text style={s.gridTitle}>
                    {t("health_profile.upcoming_appointments", {
                  ...
                  {appointments && appointments.length > 0 ? (
                    appointments
                      ...
                      .map((appt, i) => {
                        ...
                        if (
                          apptDate.toDateString() === isTodayObj.toDateString()
                        ) {
                        ...
                        return (
                          <Pressable
                            key={i}
                            style={s.apptRow}
                            onPress={() => openModal("appointment", appt)}
                          >
                            <View style={{ flex: 1 }}>
                    ...
                    <View style={{ alignItems: "center", paddingVertical: 12 }}>
                      <Text style={s.emptyGridTxt}>
                        {t("health_profile.no_appointments", {
                          defaultValue: "No appointments scheduled yet.",
                        })}
                      </Text>
                      <Pressable
                        style={s.schedBtn}
                        onPress={() => openModal("appointment")}
                      >
                        <Text style={s.schedBtnTxt}>
                          {t("health_profile.schedule_appointment", {
        ...
        <PremiumFormModal
          ...
          headerRight={
            ...
            [
              ...
              "history",
              "appointment",
            ].includes(editingType) ? (
              <Pressable
                onPress={() =>
                  handleDelete(getCollectionName(editingType), formState._id)
                }
                style={s.trashBtn}
              >
        ...
        >
          {editingType === "condition" && (
            <>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.condition_name", {
              ...
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.severity", { defaultValue: "Severity" })}
                </Text>
                <ChipSelector
                  options={severityOptions}
                  selected={formState.severity}
                  onSelect={(v) => setFormState({ ...formState, severity: v })}
                />
              </View>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.notes", { defaultValue: "Notes" })}
                  variant="multiline"
                  multiline
          ...
          {editingType === "allergy" && (
            <>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.allergy_name", {
              ...
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.reaction_details", {
                    defaultValue: "Reaction Details",
                  })}
                  variant="multiline"
                  multiline
                  value={formState.reaction}
                  onChangeText={(t) =>
                    setFormState({ ...formState, reaction: t })
                  }
          ...
          {editingType === "habits" && (
            <>
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.smoking_habits", {
                    defaultValue: "Smoking Habits",
                  })}
                </Text>
                <ChipSelector
                  options={smokeOptions}
                  selected={formState.smoking_status}
                  onSelect={(v) =>
                    setFormState({ ...formState, smoking_status: v })
                  }
                />
              </View>
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.drinking_habits", {
                    defaultValue: "Drinking Habits",
                  })}
                </Text>
                <ChipSelector
                  options={alcoholOptions}
                  selected={formState.alcohol_use}
                  onSelect={(v) =>
                    setFormState({ ...formState, alcohol_use: v })
                  }
                />
              </View>
            </>
          ...
          {editingType === "activity" && (
            <>
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.mobility_level", {
                    defaultValue: "Mobility Level",
                  })}
                </Text>
                <ChipSelector
                  options={mobilityOptions}
                  selected={formState.mobility_level}
                  onSelect={(v) =>
                    setFormState({ ...formState, mobility_level: v })
                  }
                />
              </View>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.mobility_aids", {
                    defaultValue: "Mobility Aids",
                  })}
                  value={formState.mobility_aids}
                  onChangeText={(t) =>
                    setFormState({ ...formState, mobility_aids: t })
                  }
                  placeholder={t("health_profile.mobility_aids_placeholder", {
                    defaultValue: "e.g. Cane, Walker (comma separated)",
                  })}
                />
              </View>
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.activity_intensity", {
                    defaultValue: "Activity Intensity & Duration",
                  })}
                </Text>
                <ChipSelector
                  vertical
                  options={exerciseOptions}
                  selected={formState.exercise_frequency}
                  onSelect={(v) =>
                    setFormState({ ...formState, exercise_frequency: v })
                  }
                />
              </View>
            </>
          ...
          {editingType === "identity" && (
            <>
                  ...
                  {bloodOptions.map((opt) => {
                    ...
                    return (
                      <Pressable
                        ...
                        onPress={() =>
                          setFormState({ ...formState, blood_type: opt.value })
                        }
                      >
                        <Text
                          style={[
                            s.bloodChipTxt,
                            isSelected && s.bloodChipTxtActive,
                          ]}
                        >
                          {opt.label}
                        </Text>
              ...
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.dietary_restrictions", {
                    defaultValue: "Dietary Restrictions",
                  })}
                </Text>
                <SmartInput
                  label={t("health_profile.dietary_restrictions", {
                    defaultValue: "Dietary Restrictions",
                  })}
                  variant="multiline"
                  multiline
                  ...
                  onChangeText={(t) =>
                    setFormState({ ...formState, dietary_restrictions: t })
                  }
                  placeholder={t("health_profile.dietary_placeholder", {
                    defaultValue: "e.g. Low Sodium, Diabetic, Gluten-Free",
                  })}
                />
              </View>
            </>
          ...
          {editingType === "contact" && (
            <>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.contact_name", {
              ...
              <View style={s.formGroup}>
                <SmartInput
                  label={t("caller.relationship", {
                    defaultValue: "Relationship",
                  })}
                  value={formState.relation}
                  onChangeText={(t) =>
                    setFormState({ ...formState, relation: t })
                  }
                  placeholder={t("health_profile.relationship_placeholder", {
                    defaultValue: "e.g. Daughter, Spouse",
                  })}
                />
              </View>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("caller.phone_number", {
              ...
              <View style={s.formGroup}>
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      {t("caller.emergency_contact", {
                        defaultValue: "Emergency Contact",
                      })}
                    </Text>
                    <Text style={{ fontSize: 13, color: colors.textMuted }}>
                      {t("caller.emergency_desc", {
                        defaultValue:
                          "Primary person to call in case of emergency",
                      })}
                    </Text>
                  </View>
                  <Switch
                    ...
                    onValueChange={(v) =>
                      setFormState({ ...formState, is_emergency: v })
                    }
                    trackColor={{ false: "#CBD5E1", true: colors.danger }}
                    thumbColor={
                      Platform.OS === "ios"
                        ? "#FFF"
                        : formState.is_emergency
                          ? "#FFF"
                          : "#F4F4F4"
                    }
                  />
          ...
          {editingType === "gp" && (
            <>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.doctor_name_lbl", {
                    defaultValue: "Doctor's Name",
                  })}
                  value={formState.gp_name}
                  onChangeText={(t) =>
                    setFormState({ ...formState, gp_name: t })
                  }
                  placeholder={t("health_profile.doctor_placeholder", {
              ...
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.contact_number", {
                ...
                >
                  ...
                  >
                    <Text style={{ fontSize: 16 }}>
                      {COUNTRY_CODES.find(
                        (c) => c.code === formState.gp_phoneCode,
                      )?.flag || "🇮🇳"}
                    </Text>
                    <Text
                      style={{
              ...
              <View style={s.formGroup}>
                <SmartInput
                  ...
                  autoCapitalize="none"
                  value={formState.gp_email}
                  onChangeText={(t) =>
                    setFormState({ ...formState, gp_email: t })
                  }
          ...
          {editingType === "medication" && (
            <>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.medication_name", {
                    defaultValue: "Medication Name *",
                  })}
                  value={formState.name}
                  onChangeText={(t) => setFormState({ ...formState, name: t })}
                  placeholder={t("health_profile.med_placeholder", {
                    defaultValue: "e.g. Paracetamol",
                  })}
                />
              </View>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.dosage", { defaultValue: "Dosage" })}
                  value={formState.dosage}
                  onChangeText={(t) =>
                    setFormState({ ...formState, dosage: t })
                  }
              ...
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.frequency", { defaultValue: "Frequency" })}
                </Text>
                <ChipSelector
                  options={frequencyOptions}
                  selected={formState.frequency}
                  onSelect={(v) => setFormState({ ...formState, frequency: v })}
                />
              </View>
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.times_of_day", {
                    defaultValue: "Times of Day (Select Multiple)",
                  })}
                </Text>
                <View
                ...
                >
                  {timeOptions.map((opt) => {
                    const isSelected = (formState.times || []).includes(
                      opt.value,
                    );
                    return (
                      <Pressable
                      ...
                      >
                        <Text
                          style={[
                            s.selectChipTxt,
                            isSelected && s.selectChipTxtActive,
                          ]}
                        >
                          {opt.label}
                        </Text>
              ...
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.prescribed_by", {
                    defaultValue: "Prescribed By",
                  })}
                  value={formState.prescribed_by}
                  onChangeText={(t) =>
                    setFormState({ ...formState, prescribed_by: t })
                  }
                  placeholder={t("health_profile.doctor_placeholder", {
                    defaultValue: "Doctor's Name",
                  })}
              ...
              >
                <Text style={s.formLabel}>
                  {t("health_profile.currently_active", {
                ...
                <Switch
                  ...
                  onValueChange={(val) =>
                    setFormState({ ...formState, is_active: val })
                  }
              ...
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.prescription_details", {
                    defaultValue: "Prescription Details",
                  })}
                </Text>
                <Pressable
                  style={{
                  ...
                  onPress={() =>
                    AlertManager.alert(
                      ...
                      t("health_profile.upload_coming_soon", {
                        defaultValue:
                ...
                >
                  ...
                  >
                    {t("health_profile.upload_prescription", {
          ...
          {editingType === "history" && (
            <>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.event_surgery_lbl", {
                    defaultValue: "Event / Surgery / Diagnosis *",
                  })}
                  value={formState.event}
                  onChangeText={(t) => setFormState({ ...formState, event: t })}
                  placeholder={t("health_profile.event_placeholder", {
                    defaultValue: "e.g. Knee Replacement",
                  })}
                />
              </View>
              <View style={s.formGroup}>
              ...
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.detailed_notes", {
                    defaultValue: "Detailed Notes",
                  })}
                  variant="multiline"
                  multiline
                  value={formState.notes}
                  onChangeText={(t) => setFormState({ ...formState, notes: t })}
                  placeholder={t("health_profile.surgery_notes_placeholder", {
                    defaultValue:
                      "How did the procedure go? Who was the doctor?",
                  })}
                />
              </View>
            </>
          ...
          {editingType === "vaccination" && (
            <>
              <View style={s.formGroup}>
                <SmartInput
                  label={t("health_profile.vaccine_name", {
                    defaultValue: "Vaccine Name *",
                  })}
                  value={formState.name}
                  onChangeText={(t) => setFormState({ ...formState, name: t })}
                  placeholder={t("health_profile.vaccine_placeholder", {
                    defaultValue: "e.g. Influenza, COVID-19",
                  })}
                />
              </View>
              <View style={s.formGroup}>
            ...
            <>
              <View style={s.formGroup}>
                ...
                <TextInput
                  ...
                  placeholder={t("health_profile.appt_placeholder", {
              ...
              <View style={s.formGroup}>
                <Text style={s.formLabel}>
                  {t("health_profile.doctor_specialist", {
                    defaultValue: "Doctor / Specialist Name *",
                  })}
                </Text>
                <TextInput
                  style={s.input}
                  ...
                  onChangeText={(t) =>
                    setFormState({ ...formState, doctor_name: t })
                  }
              ...
              <View style={s.formGroup}>
                ...
                <View style={{ flexDirection: "row", gap: 10 }}>
                  ...
                  >
                    ...
                    >
                      {formState.date
                        ? new Date(formState.date).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
          ...
          {editingType === "dob" && (
            <View style={s.pickerContainer}>
              ...
              <Text style={s.pickerLabel}>
                {t("common.birth_year", { defaultValue: "Birth Year" })}
              </Text>
              <ScrollView
                horizontal
                showsHorizontalScrollIndicator={false}
                  ...
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      setFormState({
                        ...formState,
                        year: y,
                        day: Math.min(formState.day || 1, maxDays),
                      });
                    }}
                    style={[
                      s.yearChip,
                      formState.year === y && s.yearChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        s.yearChipTxt,
                        formState.year === y && s.yearChipTxtActive,
                      ]}
                    >
                      {y}
                    </Text>
              ...
              <Text style={[s.pickerLabel, { marginTop: 20 }]}>
                {t("common.month", { defaultValue: "Month" })}
              </Text>
              <View style={s.monthGrid}>
                {[
                  t("months.jan", { defaultValue: "Jan" }),
                  t("months.feb", { defaultValue: "Feb" }),
                  t("months.mar", { defaultValue: "Mar" }),
                  t("months.apr", { defaultValue: "Apr" }),
                  t("months.may", { defaultValue: "May" }),
                  t("months.jun", { defaultValue: "Jun" }),
                  t("months.jul", { defaultValue: "Jul" }),
                  t("months.aug", { defaultValue: "Aug" }),
                  t("months.sep", { defaultValue: "Sep" }),
                  t("months.oct", { defaultValue: "Oct" }),
                  t("months.nov", { defaultValue: "Nov" }),
                  t("months.dec", { defaultValue: "Dec" }),
                ].map((m, i) => (
                  <Pressable
                    key={m}
                    onPress={() => {
                    ...
                    style={[
                      s.monthChip,
                      formState.month === i && s.monthChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        s.monthChipTxt,
                        formState.month === i && s.monthChipTxtActive,
                      ]}
                    >
                      {m}
                    </Text>
              ...
              >
                {Array.from(
                  ...
                  <Pressable
                    key={d}
                    onPress={() => setFormState({ ...formState, day: d })}
                    style={[s.dayChip, formState.day === d && s.dayChipActive]}
                  >
                    <Text
                      style={[
                        s.dayChipTxt,
                        formState.day === d && s.dayChipTxtActive,
                      ]}
                    >
                      {d}
                    </Text>
          ...
          <View style={s.countryModalWrap}>
            <View style={s.countryModalHeader}>
              <Text style={s.countryModalTitle}>
                {t("caller.select_country_code", {
                  defaultValue: "Select Country Code",
                })}
              </Text>
              <Pressable
                onPress={() => setCountryCodeModal(false)}
            ...
            <DateTimePicker
              ...
              onChange={(event, selectedDate) => {
                ...
                if (event.type !== "dismissed" && selectedDate) {
                  ...
                  setFormState((prev) => ({
                    ...prev,
                    [datePickerField]: current.toISOString(),
                  }));
                }
              }}
            />
              ...
              >
                {(hs?.tips || []).length === 0 ? (
                  <View style={s.tipsEmptyState}>
                    ...
                    <Text style={s.tipsEmptySub}>
                      No specific improvements needed right now. Keep up your
                      current habits.
                    </Text>
                  </View>
                ) : (
                    ...
                    return (
                      <View
                        ...
                        style={[
                          s.tipCard,
                          {
                            borderLeftColor: impactConfig.text,
                      ...
                      >
                        <View style={s.tipCardTop}>
                          ...
                          <View style={{ flex: 1 }}>
                            <Text style={[s.tipTitle, { color: "#0F172A" }]}>
                              {tip.title}
                            </Text>
                          </View>
                          <View
                            style={[
                              s.tipImpactBadge,
                              {
                                backgroundColor: impactConfig.badge,
                                borderColor: impactConfig.border,
                              },
                            ]}
                          >
                ...
                <View style={s.tipsFooter}>
                  ...
                  <Text style={s.tipsFooterTxt}>
                    Tips are personalised based on your age group, lifestyle,
                    and health data.
                  </Text>
                </View>
              </ScrollView>
        ...
        >
          {(() => {
            const hasScore =
              hsScore !== null && hsScore > 0 && completionPct >= 50;
            ...
            const getDriverPct = (driverKey) => {
              ...
              if (
                !driver ||
                typeof driver.pts !== "number" ||
                typeof driver.max !== "number" ||
                driver.max === 0
              ) {
                return 0;
              }
            ...
            const healthAgeVal =
              actualAge && healthAgeDiff !== null
                ? Math.max(18, actualAge + healthAgeDiff)
                : null;
            ...
            const firstMed =
              activeMeds && activeMeds.length > 0 ? activeMeds[0].name : null;
            const topTip =
              (hs?.tips || []).find((t) => t.impact === "high") ||
              (hs?.tips || [])[0] ||
              null;
            const coachAction = topTip
              ? topTip.title
              : firstMed
                ? `Take ${firstMed} with your next meal`
                : "Complete your health profile to unlock personalized coaching";
            const coachImpact =
              topTip?.impact === "high"
                ? "+5"
                : topTip?.impact === "medium"
                  ? "+3"
                  : "+2";
            ...
            const driverData = hsBreakdown
              ? [
                  {
                    label: "Medication",
                    pct: getDriverPct("adherence"),
                    icon: "💊",
                  },
                  {
                    label: "Lifestyle",
                    pct: getDriverPct("lifestyle"),
                    icon: "🏃",
                  },
                  { label: "Vitals", pct: getDriverPct("vitals"), icon: "🩺" },
                  {
                    label: "Conditions",
                    pct: getDriverPct("conditions"),
                    icon: "❤️",
                  },
                  {
                    label: "Preventive Care",
                    pct: getDriverPct("preventive"),
                    icon: "🛡️",
                  },
                  {
                    label: "Mobility",
                    pct: getDriverPct("mobility"),
                    icon: "🚶",
                  },
                ]
              : null;
            const driverColor = (pct) =>
              pct >= 75 ? "#10B981" : pct >= 50 ? "#F59E0B" : "#F43F5E";

            // Weakest driver for prediction
            const weakest = driverData
              ? [...driverData].sort((a, b) => a.pct - b.pct)[0]
              : null;
            const projectedBoost = weakest
              ? Math.min(8, Math.round((100 - weakest.pct) * 0.15))
              : 5;
            const projectedScore = hasScore
              ? Math.min(100, activeScoreVal + projectedBoost)
              : null;
            ...
            if (weakest) {
              const qMap = {
                ...
                Lifestyle:
                  "What active habits can help improve my lifestyle score?",
                Vitals: "How do I keep my vitals stable and healthy?",
                Conditions:
                  "What are the best ways to manage my health conditions?",
                "Preventive Care": "What preventive checks should I get done?",
                Mobility: "How can I safely improve my mobility score?",
              };
              coachQuestion =
                qMap[weakest.label] || "How can I improve my health score?";
                ...
                <View style={[s.tipsSheet, { maxHeight: "95%" }]}>
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      ...
                      >
                        Health Score{" "}
                        <Text style={{ color: "#7C3AED" }}>Coach</Text>
                      </Text>
                      <Text
                        style={{
                          fontSize: 13,
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <RefreshCw
                            size={18}
                            color={historyLoading ? "#94A3B8" : "#64748B"}
                          />
                        </Animated.View>
                      </Pressable>
                      <Pressable
                  ...
                  >
                    <Pressable
                      ...
                      style={{
                        ...
                        justifyContent: "center",
                        backgroundColor:
                          activeTab === "insights" ? "#FAF5FF" : "#F1F5F9",
                        borderWidth: 1.5,
                        borderColor:
                          activeTab === "insights" ? "#7C3AED" : "transparent",
                      }}
                    >
                      <Text
                        style={{
                    ...
                    <Pressable
                      ...
                      style={{
                        ...
                        justifyContent: "center",
                        backgroundColor:
                          activeTab === "trends" ? "#FAF5FF" : "#F1F5F9",
                        borderWidth: 1.5,
                        borderColor:
                          activeTab === "trends" ? "#7C3AED" : "transparent",
                      }}
                    >
                      <Text
                        style={{
                          ...
                          {weakest && projectedScore && (
                            <>
                              ...
                              <View style={{ gap: 4 }}>
                                ...
                                >
                                  Improving your {weakest.label.toLowerCase()}{" "}
                                  score could boost your overall health score.
                                </Text>
                                <View
                                  style={{
                                    backgroundColor: "#F3E8FF",
                        ...
                        <View style={{ gap: 20 }}>
                          ...
                          {historyPending ? (
                            ...
                            >
                              ...
                              <View style={{ flex: 1 }}>
                                ...
                                >
                                  We are analyzing your 30-day historical logs.
                            ...
                            (!healthHistory.history ||
                              healthHistory.history.length < 5) && (
                              <View
                                style={{
                                  backgroundColor: "#F8FAFC",
                                  borderRadius: radius.lg,
                              ...
                              >
                                ...
                                <View style={{ flex: 1 }}>
                                  ...
                                  >
                                    Log medications and vitals for at least 5
                                    days to unlock personalized AI predictions,
                                    health momentum, and forecasting.
                                  </Text>
                                </View>
                              </View>
                            )
                            ...
                            <View style={{ flexDirection: "row", gap: 12 }}>
                              ...
                              >
                                <View>
                                  ...
                                  <Text
                                    style={{
                                      ...
                                      ...FONT.heavy,
                                      color:
                                        healthHistory.predictive_health.momentum
                                          .direction === "improving"
                                          ? "#10B981"
                                          : healthHistory.predictive_health
                                                .momentum.direction ===
                                ...
                                <View style={{ marginTop: 8 }}>
                                  ...
                                  >
                                    ...
                                    {healthHistory.predictive_health.momentum
                                      .score_change_30d >= 0
                                      ? `+${healthHistory.predictive_health.momentum.score_change_30d}`
                                      : healthHistory.predictive_health.momentum
                                          .score_change_30d}{" "}
                                    pts vs prior month
                                  </Text>
                                  ...
                                  >
                                    30d Adherence:{" "}
                                    {healthHistory.predictive_health.momentum
                                      .adherence_change_30d >= 0
                                      ? `+${healthHistory.predictive_health.momentum.adherence_change_30d}`
                                      : healthHistory.predictive_health.momentum
                                          .adherence_change_30d}
                                    % vs prior month
                                  </Text>
                                </View>
                              </View>
                              ...
                              >
                                ...
                                >
                                  ...
                                  {healthHistory.predictive_health.consistency
                                    .score >= 80 && (
                                    <View
                                      style={{
                                        backgroundColor: "#FAF5FF",
                          ...
                          <View style={s.timelineContainer}>
                            <Text style={s.timelineHeading}>
                              Health Journey Replay
                            </Text>
                            {timeline.length === 0 ? (
                              <Text style={s.timelineEmpty}>
                                Maintain consistency to unlock achievements &
                                milestones!
                              </Text>
                            ) : (
            ...
            <Pressable
              ...
              onPress={() => {
                if (vitalsPickerType === "height") {
                  setFormState((prev) => ({
                    ...prev,
                    height_cm: vitalsTempHeightCm,
                  }));
                } else {
                  setFormState((prev) => ({
                    ...prev,
                    weight_kg: vitalsTempWeightKg,
                  }));
                }
            ...
            <View style={s.pickerSheet}>
              ...
              <View style={s.pickerToggleBar}>
                <Pressable
                  style={[
                    s.pickerToggleBtn,
                    vitalsUnitSystem === "metric" && s.pickerToggleBtnActive,
                  ]}
                  onPress={() => {
                    LayoutAnimation.configureNext(
                      LayoutAnimation.Presets.easeInEaseOut,
                ...
                >
                  <Text
                    style={[
                      s.pickerToggleText,
                      vitalsUnitSystem === "metric" && s.pickerToggleTextActive,
                    ]}
                  >
                    Metric
                  </Text>
                ...
                <Pressable
                  style={[
                    s.pickerToggleBtn,
                    vitalsUnitSystem === "imperial" && s.pickerToggleBtnActive,
                  ]}
                  onPress={() => {
                    LayoutAnimation.configureNext(
                      LayoutAnimation.Presets.easeInEaseOut,
                ...
                >
                  <Text
                    style={[
                      s.pickerToggleText,
                      vitalsUnitSystem === "imperial" &&
                        s.pickerToggleTextActive,
                    ]}
                  >
                    Imperial
                  </Text>
                ...
                {vitalsPickerType === "height" ? (
                        ...
                        <View style={{ width: 85 }}>
                          <TactileWheelPicker
                            ...
                            selectedValue={Math.max(
                              ...
                              Math.min(
                                8,
                                Math.floor(vitalsTempHeightCm / 2.54 / 12),
                              ),
                            )}
                            onValueChange={(newFt) => {
                              const realInches = vitalsTempHeightCm / 2.54;
                      ...
                      >
                        <Pressable
                          ...
                          onPress={() => {
                            ...
                            const ft = Math.floor(realInches / 12);
                            const inch =
                              Math.round(realInches % 12) === 12
                                ? 0
                                : Math.round(realInches % 12);
                            const newInch = inch === 0 ? 11 : inch - 1;
                            const newFt = inch === 0 ? Math.max(3, ft - 1) : ft;
                            const cm = Math.round(
                              (newFt * 12 + newInch) * 2.54,
                            );
                            setVitalsTempHeightCm(cm);
                            Haptics.selectionAsync().catch(() => {});
                          }}
                        ...
                        <View style={{ width: 85 }}>
                          <TactileWheelPicker
                            ...
                            selectedValue={Math.max(
                              ...
                              Math.min(
                                11,
                                Math.round((vitalsTempHeightCm / 2.54) % 12) ===
                                  12
                                  ? 0
                                  : Math.round(
                                      (vitalsTempHeightCm / 2.54) % 12,
                                    ),
                              ),
                            )}
                  ...
                  >
                    <Pressable
                      style={s.stepBtn}
                      onPress={() => {
                        const currentLbs = Math.round(
                          vitalsTempWeightKg / 0.45359237,
                        );
                    ...
                    <View style={{ width: 140 }}>
                      <TactileWheelPicker
                        data={Array.from({ length: 491 }, (_, i) => ({
                          label: `${i + 60} lbs`,
                          value: i + 60,
                        }))}
                    ...
                    <Text style={s.bmiPreviewValue}>
                      {(() => {
                        const heightToUse =
                          vitalsPickerType === "height"
                            ? vitalsTempHeightCm
                            : formState.height_cm || 170;
                        const weightToUse =
                          vitalsPickerType === "weight"
                            ? vitalsTempWeightKg
                            : formState.weight_kg || 70;
                  ...
                  <View style={{ alignItems: "flex-end" }}>
                    {(() => {
                      ...
                      return (
                        ...
                        >
                          <View
                            style={[s.bmiCategoryDot, { backgroundColor: col }]}
                          />
                          <Text style={[s.bmiCategoryText, { color: col }]}>
                            {cat}
                          </Text>
                        </View>
                      );
