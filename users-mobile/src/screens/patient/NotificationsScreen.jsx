...
export default function NotificationsScreen({ navigation }) {
  ...
  const staggerAnims = useRef(
    [...Array(STAGGER_COUNT)].map(() => new Animated.Value(0)),
  ).current;
  ...
  const groupAnim = (i) => ({
    ...
    transform: reduceMotion
      ...
      : [
          {
            translateY: staggerAnims[
              Math.min(i, STAGGER_COUNT - 1)
            ].interpolate({ inputRange: [0, 1], outputRange: [14, 0] }),
          },
        ],
  });
  ...
  const handleBatteryAction = async () => {
    try {
      ...
      // Dismiss prompt so it doesn't stay visible like a persistent error once configured
      await handleDismissBatteryPrompt();
    } catch (e) {
    ...
    <View style={s.container}>
      ...
      <View style={s.tabsWrap}>
        <View style={s.tabsBg}>
          ...
          >
            <Text
              style={[s.tabText, activeTab === "unread" && s.tabTextActive]}
            >
              Unread
            </Text>
        ...
        {showBatteryPrompt && (
          <View style={s.batteryBanner}>
            <View style={s.batteryBannerTop}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={s.batteryTitle}>
                  Ensure Reminders Fire On Time
                </Text>
                <Text style={s.batteryDesc}>
                  Android may delay or block your medication reminders to save
                  battery. Tap below to disable battery optimization for
                  CareMyMed.
                </Text>
              </View>
              <Pressable
                style={s.batteryClose}
                onPress={handleDismissBatteryPrompt}
                hitSlop={15}
              >
        ...
        {loading ? (
          ...
          GROUP_ORDER.map((group, groupIdx) => {
            ...
            return (
              <Animated.View
                key={group}
                style={[s.groupSection, groupAnim(groupIdx)]}
              >
    ...
    <View style={s.cardBody}>
      <View style={s.cardTopRow}>
        <Text
          style={[
            s.cardName,
            item.isBackend && !item.isRead && s.cardNameUnread,
          ]}
          numberOfLines={1}
        >
          {item.name}
      ...
      <Text style={s.cardAction} numberOfLines={2}>
        {item.action}
      </Text>
    </View>

    <View style={[s.actionBtn, { backgroundColor: item.bg }]}>
      <Text style={[s.actionBtnTxt, { color: item.color }]}>
        {item.actionTxt}
      </Text>
    </View>
  </Pressable>
