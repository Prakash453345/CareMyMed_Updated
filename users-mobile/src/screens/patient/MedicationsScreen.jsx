  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        ...
        >
          {t(`time_slots.${slot}`, { defaultValue: label })}
        </Text>
      </View>
      {callTime ? (
        <View
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {t("medications.take", { defaultValue: "TAKE" })}
          </Text>
        </Animated.View>
      </Pressable>
    );
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {t("medications.snooze", { defaultValue: "SNOOZE" })}
          </Text>
        </Animated.View>
      </Pressable>
    );
    ...
    <Modal visible transparent animationType="fade">
      ...
      >
        ...
        >
          ...
          >
            {t("medications.set_time", { defaultValue: "Set Time" })}
          </Text>
          <Text
            style={{
              fontSize: 13,
          ...
          >
            {t("medications.scroll_time", {
              defaultValue: "Scroll to choose your preferred time",
            })}
          </Text>
...
export default function MedicationsScreen({ navigation, route }) {
    ...
    async (isRefresh = false) => {
      try {
        ...
        try {
          ...
          console.warn(
            "Failed to fetch temporary medications:",
            tempErr.message,
          );
        }
  ...
  const handleMedIconPress = useCallback((med) => {
    ...
    if (slotStart !== undefined && hour < slotStart) {
      const msgs = {
        morning: {
          title: t("medications.early_bird_title", {
            defaultValue: "Easy there, early bird! 🐦",
          }),
          body: t("medications.early_bird_body", {
            defaultValue: "Morning meds aren't due yet. Come back after 5 AM!",
          }),
        },
        afternoon: {
          title: t("medications.not_yet_title", {
            defaultValue: "Not yet! ☀️",
          }),
          body: t("medications.not_yet_afternoon", {
            defaultValue: "Afternoon meds unlock from 11 AM. Patience!",
          }),
        },
        evening: {
          title: t("medications.hold_on_title", {
            defaultValue: "Hold on! 🌅",
          }),
          body: t("medications.not_yet_evening", {
            defaultValue: "Evening meds are available after 4 PM.",
          }),
        },
        night: {
          title: t("medications.not_night_yet_title", {
            defaultValue: "It's not night yet! 🌙",
          }),
          body: t("medications.not_yet_night", {
            defaultValue: "Night meds start from 7 PM. Enjoy your day first!",
          }),
        },
      };
      const msg = msgs[med.type] || {
        title: t("medications.not_yet_generic_title", {
          defaultValue: "Not yet! ⏰",
        }),
        body: t("medications.not_yet_generic", {
          defaultValue: "This medication isn't due yet.",
        }),
      };
      AlertManager.alert(msg.title, msg.body, [
        {
          text: t("common.got_it", { defaultValue: "Got it! 😊" }),
          style: "default",
        },
      ]);
      return;
  ...
  const handleConfirmToggle = async () => {
    ...
    try {
      ...
      showToast(
        t("common.sync_failed", { defaultValue: "Sync Failed" }),
  ...
  const submitAddTempMed = async () => {
    ...
    try {
      ...
      if (savedMed) {
        ...
        setTempMeds((prev) =>
          prev.map((m) => (m._id === tempId ? savedMed : m)),
        );
        setTempMedForm({
          name: "",
          dosage: "",
  ...
  const handleSnooze = async (med) => {
    try {
      ...
      if (finalStatus !== "granted") {
        AlertManager.alert(
          ...
          t("medications.enable_notifications", {
            defaultValue: "Please enable notifications to snooze medications.",
          }),
      ...
      showToast(
        t("medications.snoozed", { defaultValue: "Snoozed 30 min" }),
        `${t("medications.remind_you_about", { defaultValue: "We'll remind you about" })} ${med.name}.`,
        "info",
      );
    } catch (err) {
  ...
  const handleSavePreferences = async () => {
    ...
    try {
      ...
      showToast(
        t("common.error", { defaultValue: "Error" }),
        t("medications.save_prefs_failed", {
  ...
  const submitRefill = async () => {
    ...
    try {
      ...
      showToast(
        t("common.success", { defaultValue: "Success" }),
        t("medications.refill_success", {
          defaultValue: "Medication supply refilled!",
        }),
        "success",
      );
      ...
      showToast(
        t("common.error", { defaultValue: "Error" }),
        t("medications.refill_failed", {
          defaultValue: "Failed to refill medication.",
        }),
        "error",
      );
    } finally {
  ...
  const handleUploadPrescription = async () => {
    try {
      ...
      if (status !== "granted") {
        showToast(
          t("common.permission_needed", { defaultValue: "Permission needed" }),
          t("medications.camera_req", {
